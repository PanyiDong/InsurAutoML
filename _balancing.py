...
def is_imbalance(data, threshold, value = False) :
    ...
    for _column in features :
        ...
        for _value in unique_values :
            if len(data.loc[data[_column] == _value, _column]) / len(data[_column]) > threshold :
                if value :
...
class ExtremeClass() :
    ...
    def cut(self, X) :
        ...
        for _column in features :
            ...
            for _value in unique_values :
                if len(X.loc[X[_column] == _value, _column]) / len(X) >= self.extreme_threshold :
...
class EditedNearestNeighbor() :
    ...
    def fit_transform(self, X, y = None) :
        ...
        if (self.k % 2) == 0 :
            warnings.warn('Criteria of majority better select odd k nearest neighbors, get {}.'.format(self.k))
...
class CondensedNearestNeighbor() :
    ...
    def _fit_transform(self, X) :
        ...
        while is_imbalance(X[[_imbalanced_feature]], self.imbalance_threshold) :
            ...
            neigh.fit(_subset.loc[:, _subset.columns != _imbalanced_feature], _subset[_imbalanced_feature])
            y_predict = neigh.predict(X.loc[~X.index.isin(list(_subset.index)), X.columns != _imbalanced_feature])
            y_true = X.loc[~X.index.isin(list(_subset.index)), X.columns == _imbalanced_feature].values.T[0]
...
class Smote() :
    ...
    the link sample will be chosen from these k nearest neigbhors

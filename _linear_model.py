...
class LinearRegression(object) :
    ...
    def Gradient_Descent(self, x, y, backtracking, method, ini_beta, plot) :
    ...
    def predict(self, x) :
        if len(x[0]) != len(LinearRegression.beta) - 1:
            raise ValueError('Length of inputs not matching regression model!')
        
        x0 = []
        for row in x:
            x0.append(np.insert(row, 0, 1).tolist())
        return [round(np.dot(np.array(item).T, LinearRegression.beta)[0], 4) for item in x0]
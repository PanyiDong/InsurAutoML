...
class feature_selection() :
    ...
    def __init__(
        ...
        self.time_left_for_this_task = time_left_for_this_task # total time in seconds to find and tune the models
    ...
    def database_test(self, database) :
        ...
        for _mol in self.feature_mol :
            if _mol not in [*_feature_mol] :
                raise ValueError('{0} not avaiable! All possible models are: {1}.'.format(_mol, [*_feature_mol]))
    ...
    def _skip_data_test(self, data, data_name, base_info) :
        ...
        response = next(item for item in data_info['property'] if item['task_type'] == self.task_type)['response']
        ...
        if response == 'None' :
            ...
            data_info_file.write('Number of samples: {},\nNumber of features: {}\n' \
                .format(data[features].shape[0], data[features].shape[1]))
            ...
            X_train, X_test, y_train, y_test = train_test_split(new_data, data[[response]], \
                test_size = self.test_perc, random_state = self.seed)
    ...
    def _data_test(self, data, data_name, base_info) :
        ...
        if response == 'None' :
            ...
            for _mol in self.feature_mol :
                ...
                # write basic information to data_info file
                new_info_file = open('{}/{}_info.txt'.format(new_info_folder, _mol), 'a')
                new_info_file.write('Data name: {}\n'.format(data_name))
                new_info_file.write('Task type: {}\n'.format(self.task_type))
                new_info_file.write('Number of train samples: {},\nNumber of test samples: {},\nNumber of features: {}\n' \
                    .format(X_train.shape[0], X_test.shape[0], X_train.shape[1]))
                ...
                result_file = open('{}/{}_result.txt'.format(new_info_folder, _mol), 'a')
                result_file.write('Train accuracy: {}\n'.format(accuracy_score(y_train, fitting)))
                result_file.write('Train MAE: {}\n'.format(mean_absolute_error(y_train, fitting)))
                result_file.write('Train MSE: {}\n'.format(mean_squared_error(y_train, fitting)))
                result_file.write('Test accuracy: {}\n'.format(accuracy_score(y_test, predictions)))
